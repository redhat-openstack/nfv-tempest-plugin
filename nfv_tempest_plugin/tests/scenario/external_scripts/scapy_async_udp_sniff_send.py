...
from scapy.all import IP, UDP, Raw, send, AsyncSniffer  # noqa
...
def construct_packet(dest_ip, port):
    return IP(dst=dest_ip)/UDP(dport=int(port),  # noqa
                               sport=int(port))/Raw(load="packet")  # noqa
...
def main():
    args = parse_args()
    packet = construct_packet(args.dest, args.port)
