...
class TestDeriveParamsScenarios(base_test.BaseTest):
    ...
    # Gets the CPU model
    def _get_cpu_model(self, hypervisor_ip):
    ...
    # Gets the CPU flages
    def _get_cpu_flags(self, hypervisor_ip):
    ...
    def _get_kernel_args(self, hypervisor_ip, hugepage_alloc_perc=50):
        kernel_args = {}
        cpu_flags = self._get_cpu_flags(hypervisor_ip)
        ...
        cpu_model = self._get_cpu_model(hypervisor_ip)
    ...
    def _get_node_dpdk_status(self, hypervisor_ip):
        cmd = "sudo ovs-vsctl get Open_vSwitch . dpdk_initialized"
        output = self._run_command_over_ssh(hypervisor_ip, cmd)
        if output and 'true' in output.strip('\n'):
            return True
        else:
            return False

    def _get_node_sriov_status(self, hypervisor_ip):
        cmd = 'sudo cat /etc/puppet/hieradata/service_names.json'
        output = self._run_command_over_ssh(hypervisor_ip, cmd)
        service_names = json.loads(output)
    ...
    def test_derived_parameters(self, test='derive_params'):
        ...
        dpdk_enabled_status = self._get_node_dpdk_status(hypervisor_ip)
        sriov_enabled_status = self._get_node_sriov_status(hypervisor_ip)
        derived_params = self._derive_parameters(hypervisor_ip,
                                                 dpdk_enabled_status,
                                                 sriov_enabled_status)
