...
class TestNfvOffload(base_test.BaseTest):
    ...
    def run_conntrack_testcase(self, test, protocol):
        ...
        for server in servers:
            self.copy_file_to_remote_host(server['fip'],
                                          server['private_key'],
                                          self.instance_user,
                                          files='scapy_async_sniff_send.py',
                                          src_path=script_dir,
                                          dst_path='/tmp/',
                                          timeout=60)
    ...
    def check_conntrack(self, srv_pair, protocol):
        ...
        # execute tcpdump in representor port in both hypervisors
        scapy_port = random.randrange(8000, 9000)
        ...
        cmd_first_vm = ("nohup sudo python3 /tmp/scapy_async_sniff_send.py -s "
                        "{iface} -d {ip} -p {l4_port}"
                        .format(iface=local_interface_first_vm,
                                ip=ip_address_second_vm,
                                l4_port=scapy_port))
        cmd_second_vm = ("nohup sudo python3 /tmp/scapy_async_sniff_send.py -s"
                         " {iface} -d {ip} -p {l4_port}"
        ...
        if protocol == 'udp':
            cmd_first_vm += ' -u'
            cmd_second_vm += ' -u'
        srv_pair[0]['server']['ssh_source'].exec_command(cmd_first_vm+"&")
        srv_pair[1]['server']['ssh_source'].exec_command(cmd_second_vm+"&")
        for sv in srv_pair:
            errors += \
