    ...
    def test_offload_ovs_flows(self, test='offload_flows'):
        ...
        for server in servers[1:]:
            ...
            for provider_network in server['provider_networks']:
                ...
                errors_found += self.check_offload(srv_pair, "icmp",
                                                   offload_injection_time)
                errors_found += self.check_offload(srv_pair, "tcp",
                                                   offload_injection_time)
                errors_found += self.check_offload(srv_pair, "udp",
                                                   offload_injection_time)
    ...
    def check_offload(self, srv_pair, protocol, duration):
        ...
        for srv_item in srv_pair:
            ...
            msg_header = "network_type {}, hypervisor {}, vm ip {} " \
                         "protocol {}.".format(
                srv_item['network']['provider:network_type'],
                srv_item['server']['hypervisor_ip'],
            ...
            output = shell_utils.stop_tcpdump(
                srv_item['server']['hypervisor_ip'],
            ...
            if protocol == "icmp":
                ...
                if udp_packets != 1:
                    errors.append("{} There should be a single UDP packet in "
                                  "representor port. {}".format(msg_header,
                                                                udp_packets))
            elif protocol == "tcp":
