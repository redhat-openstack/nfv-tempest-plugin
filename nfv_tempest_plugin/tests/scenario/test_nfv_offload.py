    ...
    def test_offload_ovs_flows(self, test='offload_flows'):
        ...
        for server in servers[1:]:
            for provider_network in server['provider_networks']:
                ...
                # tcpdump to check packets in representor port
                vf_nic = shell_utils.get_vf_from_mac(
                    provider_network['mac_address'],
                ...
                for hypervisor in hypervisors:
                    out = shell_utils.run_command_over_ssh(hypervisor,
                                                           cmd_flows)
                    msg = 'Hypervisor {} has no offloaded flows in OVS'\
                        .format(hypervisor)
                    self.assertNotEmpty(out, msg)
                    LOG.info('Hypervisor {} has offloaded flows in OVS'.
                             format(hypervisor))
