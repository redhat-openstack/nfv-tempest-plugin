    ...
    def test_offload_ovs_flows(self, test='offload_flows'):
        ...
        for server in servers[1:]:
            for provider_network in server['provider_networks']:
                ...
                # get network in the server executing ping
                source_network = next\
                    (item for item in servers[0]['provider_networks']
                     if item["network_id"] == provider_network["network_id"])
                ...
                # continuous ping
                shell_utils.continuous_ping(provider_network['ip_address'],
                                            duration=600,
                                            ssh_client_local=ssh_source)
                LOG.info('Run continuous ping from {} to {}'.
                         format(servers[0]['fip'],
                                provider_network['ip_address']))

                # Execute ping for a while, we need several ping requests/replies
                ...
                for srv_item in srv_pair:
                    ...
                    self.assertTrue\
                        ('capabilities' in port['binding:profile'] and
                         'switchdev' in
