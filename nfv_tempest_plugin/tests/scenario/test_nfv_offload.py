...
class TestNfvOffload(base_test.BaseTest):
    ...
    def test_offload_ovs_flows(self, test='offload_flows'):
        ...
        offload_injection_time = int(CONF.nfv_plugin_options.offload_injection_time)
        ...
        for server in servers[1:]:
            ...
            for provider_network in server['provider_networks']:
                ...
                errors_found += self.check_offload(srv_pair, "icmp", offload_injection_time)
                errors_found += self.check_offload(srv_pair, "tcp", offload_injection_time)
                errors_found += self.check_offload(srv_pair, "udp", offload_injection_time)
    ...
    def check_offload(self, srv_pair, protocol, duration):
        ...
        errors = []
        # sleep several seconds so that flows generated checking provider network
        # connectivity during resource creation are removed. Timeout for flows
        # deletion is around 10 seconds
c
