    ...
    def check_offload(self, srv_pair, protocol, duration):
        ...
        if protocol == "icmp":
            ...
            shell_utils.iperf_server(srv_pair[0]['network']['ip_address'],
                                     8231, duration, protocol, srv_pair[0]['ssh_source'])
            shell_utils.iperf_client(srv_pair[1]['network']['ip_address'],
                                     8231, duration, protocol, srv_pair[1]['ssh_source'])
        ...
        for srv_item in srv_pair:
            ...
            if ('capabilities' not in port['binding:profile'] or
                'switchdev' not in port['binding:profile']['capabilities']):
                errors_found.append("hypervisor {}, vm ip {}, protocol {} . "
            ...
            LOG.info('Executed on {}: {}'.format(server['hypervisor_ip'],
                                                 stop_cmd))
            output = shell_utils.run_command_over_ssh(srv_item['server']['hypervisor_ip'],
                                                      stop_cmd)
            ...
            if protocol == "icmp":
                ...
                if icmp_requests != 1 or icmp_replies != 1:
                    errors_found.append('hypervisor {}, vm ip {} protocol {}.'
                                        ...
                                        format(srv_item['server']['hypervisor_ip'],
                                               srv_item['network']['ip_address'],
                                               protocol, icmp_requests, icmp_replies))
            elif protocol == "udp":
