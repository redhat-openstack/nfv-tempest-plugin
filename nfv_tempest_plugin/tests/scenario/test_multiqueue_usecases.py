...
class TestMultiqueueScenarios(base_test.BaseTest):
    ...
    def autobalance_functionality(self, servers_dict, action):
        ...
        cmd_load_threshold = 'sudo ovs-vsctl --no-wait set open_vSwitch . other_config:pmd-auto-lb-load-threshold="70"'
        cmd_load_improvement_threshold = 'sudo ovs-vsctl --no-wait set open_vSwitch . other_config:pmd-auto-lb-improvement-threshold="50"'
        cmd_load_rebal_interval = 'sudo ovs-vsctl --no-wait set open_vSwitch . other_config:pmd-auto-lb-rebal-interval="10"'

        LOG.info('Reset load_threshold cmd {}'.format(cmd_load_threshold))
        shell_utils.run_command_over_ssh(servers_dict['testpmd']['hypervisor_ip'],cmd_load_threshold)
        LOG.info('Reset load_threshold cmd {}'.format(cmd_load_improvement_threshold))
        shell_utils.run_command_over_ssh(servers_dict['testpmd']['hypervisor_ip'],cmd_load_improvement_threshold)
        LOG.info('Reset load_threshold cmd {}'.format(cmd_load_rebal_interval))
        shell_utils.run_command_over_ssh(servers_dict['testpmd']['hypervisor_ip'],cmd_load_rebal_interval)
