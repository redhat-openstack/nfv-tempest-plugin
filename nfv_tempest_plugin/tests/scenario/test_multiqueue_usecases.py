...
class TestMultiqueueScenarios(base_test.BaseTest):
    ...
    def test_multiqueue_deployment(self, test='multiqueue_deployment'):
        ...
        result = []
        cmd_autob = 'sudo ovs-vsctl --format=json get ' \
                    'open_vswitch . other_config'
        cmd_inter = ' ovs-vsctl  list Interface {} | ' \
        ...
        for hypervisor_ip in hypervisors:
            ...
            output_autob = shell_utils.run_command_over_ssh(hypervisor_ip,
                                                            cmd_autob)
            ovs_data = json.loads(output_autob)
            try:
                for key in autob_dict.keys():
                    ovs_data_filt[key] = (
                        ovs_data['data'][0][ovs_data['headings'].index(key)])
            except Exception:
                pass
            ...
            for port in multiq_dict:
                output_inter = shell_utils.run_command_over_ssh(
                    hypervisor_ip, cmd_inter.format(port['port']))
