...
class TestMultiqueueScenarios(base_test.BaseTest):
    ...
    def test_multiqueue_autobalance(self, test='multiqueue_autobalance'):
        ...
        # if rebalance took place, check that all cpu usage is under the
        # threshold
        if rebalance:
            # After rebalacing, pmd cpu usage takes some seconds to be
            # available. Data should be available in less than 10 seconds
