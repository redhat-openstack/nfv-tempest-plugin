...
class TestNfvBasic(baremetal_manager.BareMetalManager):
    ...
    def test_emulatorpin(self, test='emulatorpin'):
        ...
        if self.test_setup_dict['emulatorpin']['config_dict'][0]:
            epin = self.test_setup_dict['emulatorpin']['config_dict'][0]
            config_path = epin['config_path']
            check_section = epin['check_section']
            check_value = epin['check_value']
