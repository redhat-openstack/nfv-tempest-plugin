...
class TestNfvBasic(base_test.BaseTest):
    ...
    def test_hypevisor_tuning(self):
        ...
        packages = next((item['packages'] for item in tuning_details
                         if 'packages' in item), None)
        services = next((item['services'] for item in tuning_details
                         if 'services' in item), None)
        tuned_profiles = next((item['tuned_profiles'] for item in
                               tuning_details if 'tuned_profiles'
                               in item), None)
        kernel_args = next((item['kernel_args'] for item in tuning_details
                            if 'kernel_args' in item), None)
