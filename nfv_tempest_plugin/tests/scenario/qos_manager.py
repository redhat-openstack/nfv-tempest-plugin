...
class QoSManagerMixin(object):
    ...
    def run_iperf_test(self, qos_policies=[], servers=[], key_pair=[],
        ...
        for qos_policy_id in range(len(qos_policies)):
            for server_id in range(len(self.servers) - 1):
                self.update_port(
                    tested_ports[server_id][0],
                    **{'qos_policy_id': qos_policies[qos_policy_id]['id']}
                )
