...
class QoSManagerMixin(object):
    ...
    def run_iperf_test(self, qos_policies=[], servers=[], key_pair=[],
        ...
        tested_ports = [shell_utils.find_vm_interface(p
        ...
        shell_utils.iperf_client(ip_addr, 5102, 60, "tcp", ssh_source1)
        shell_utils.iperf_client(ip_addr, 5101, 60, "tcp", ssh_source2)
    ...
    def collect_iperf_results(self, qos_rules_list=[],
        ...
        for index in range(srv.SERVER):
            ...
            if 'min_kbps' in qos_rules_list[srv(index)]:
                qos_type = 'min_kbps'
            self.calculate_deviation(test_type=qos_type, result_number=float(rate),
