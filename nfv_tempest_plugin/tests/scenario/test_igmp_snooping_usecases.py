...
class TestIgmpSnoopingScenarios(base_test.BaseTest):
    ...
    def test_igmp_snooping(self, test='igmp_snooping'):
        ...
        mcast_port = '10000'
        mcast_msg = ''.join(random.choice(string.ascii_lowercase)
                            for i in range(20))
        mcast_output = '/tmp/output'
        mcast_packets = 200
        mcast_packets_tolerance = 250
        get_mcast_results = 'cat {} | sort | uniq -c'.format(mcast_output)
        # not able to use pgrep/pkill -f multicat_traffic.py. With -f option,
        # it fails
        kill_cmd = "pids=$(ps -e -o \"cmd:50 \" -o \"|%p\" | " \
            "awk -F '|' '$1 ~ /multicast/ { print $2}');" \
            "if [[ ! -z $pids ]];then sudo kill -9 $pids;fi;"
        receive_cmd = kill_cmd + \
            'sudo python /usr/local/bin/multicast_traffic.py -r ' \
            '-g {0} -p {1} -c {2} > {3} 2>&1 &'.format(mcast_group,
                                                       mcast_port,
                                                       mcast_packets,
                                                       mcast_output)
        send_cmd = kill_cmd + \
            'sudo python /usr/local/bin/multicast_traffic.py -s -g' \
        ...
        for role in [{'role': 'listener', 'cmd': receive_cmd},
            ...
            # check that multicast groups were generated/removed properly
            # in the compute
            if role['role'] == 'listener':
                mcast_groups = self.get_ovs_multicast_groups("br-int",
                                                             mcast_group)
        ...
        for server in servers:
            ...
            LOG.info('{} - {} Multicast Traffic stats, tx_packets: {}, ' +
                     'rx_packets {}.'.format(server['mcast_srv'],
                                             server['fip'],
                                             tx_packets_mcast,
                                             rx_packets_mcast))
            LOG.info('{} - {} Management Traffic stats, tx_packets: {}, ' +
                     'rx_packets {}.'.format(server['mcast_srv'],
                                             server['fip'],
                                             tx_packets_mgmt,
                                             rx_packets_mgmt))
            if server['mcast_srv'] == "traffic_runner":
                if not (rx_packets_mcast >= mcast_packets and
                        rx_packets_mcast <= mcast_packets_tolerance):
                    errors.append("No traffic in traffic runner: {}. ".format(
                        rx_packets_mcast))
            else:
                if not (tx_packets_mcast >= mcast_packets and
                        tx_packets_mcast <= mcast_packets_tolerance):
