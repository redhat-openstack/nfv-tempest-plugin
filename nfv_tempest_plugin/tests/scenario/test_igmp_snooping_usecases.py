...
class TestIgmpSnoopingScenarios(base_test.BaseTest):
    ...
    def test_igmp_snooping(self, test='igmp_snooping'):
        ...
        hypervisors = dict([ [server['hypervisor_ip'],[]] for server in servers])
        [hypervisors[server['hypervisor_ip']].append(index) for index,server in enumerate(servers)]

        if [ len(hypervisors[hyp]) for hyp in hypervisors.keys()] != [3,3]:
            raise ValueError('The test requires 2 hypervisors and 3 vms in each one.')
        ...
        for group in mcast_groups:
            ...
            group['msg'] = ''.join(random.choice(string.ascii_lowercase)
                                   for i in range(20))
        # servers to be used
        servers[hypervisors.items()[0][1][0]]['mcast'] = [{'role': 'traffic-runner',
                                           'group': 0}]
        servers[hypervisors.items()[0][1][1]]['mcast'] = [{'role': 'listener',º 'group': 0}]
        servers[hypervisors.items()[0][1][2]]['mcast'] = [{'role': 'listener', 'group': 1}]
        servers[hypervisors.items()[0][1][0]]['mcast'] = [{'role': 'traffic-runner', 'group': 1}]
        servers[hypervisors.items()[1][1][1]]['mcast'] = [{'role': 'listener', 'group': 0}]
        servers[hypervisors.items()[1][1][2]]['mcast'] = [{'role': 'listener', 'group': 1}]
    ...
    def test_igmp_multiple_senders(self, test='igmp_multiple_senders'):
        """Test igmp snooping. Multiple senders to the same group
        ...
        for group in mcast_groups:                                              
            ...
            group['msg'] = ''.join(random.choice(string.ascii_lowercase)        
                                   for i in range(20))                          
        # servers to be used                                                    
        servers[hypervisors.items()[0][1][0]]['mcast'] = [{'role': 'traffic-runner', 'group': 0}]
        servers[hypervisors.items()[0][1][1]]['mcast'] = [{'role': 'listener', 'group': 0}]
        servers[hypervisors.items()[0][1][2]]['mcast'] = [{'role': 'listener', 'group': 0}]
        servers[hypervisors.items()[1][1][0]]['mcast'] = [{'role': 'traffic-runner', 'group': 0}]
        servers[hypervisors.items()[1][1][1]]['mcast'] = [{'role': 'listener', 'group': 0}]
        servers[hypervisors.items()[1][1][2]]['mcast'] = [{'role': 'listener', 'group': 0}]
