...
class TestIgmpSnoopingScenarios(base_test.BaseTest):
    ...
    def test_deployment_igmp_snooping(self, test='deployment_igmp_snooping',
        ...
        cmd = 'sudo ovs-vsctl list bridge br-int | ' \
              'sed \'s/, /\\n/g\'  | sed \'s/=\"/: /g\' | sed \'s/\"}//g\' |' \
              'egrep "^mcast_snooping_enable|' \
              '^mcast-snooping-disable-flood-unregistered"'
        output = self._run_command_over_ssh(hypervisor_ip, cmd).split('\n')
        ovs_data = {}
        for i in range(len(output)):
            data = output[i].split(': ')
            if len(data) == 2:
                ovs_data[data[0]] = data[1]

        result = []
        checks = {'mcast_snooping_enable': 'true',
                  'mcast-snooping-disable-flood-unregistered': 'true'}
        diff_checks_cmd = set(checks.keys()) - set(ovs_data.keys())
        ...
        for check in checks:
            if check not in diff_checks_cmd:
                if ovs_data[check] != checks[check]:
