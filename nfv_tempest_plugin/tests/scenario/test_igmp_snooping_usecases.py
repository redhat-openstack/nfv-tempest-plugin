...
class TestIgmpSnoopingScenarios(base_test.BaseTest):
    ...
    def test_igmp_snooping(self, test='igmp_snooping'):
        ...
        for server in servers:
            ...
            for port in ports:
                if port['fixed_ips'][0]['ip_address'] not in management_ips:
                    server['multicast_port'] = port['binding:vif_details']['vhostuser_socket'].split('/')[-1]
                    port_list.append(server['multicast_port'])
        ...
        for server in servers:
            tx_packets=stats_end[server['multicast_port']]["tx_packets"]
            rx_packets=stats_end[server['multicast_port']]["rx_packets"]
            LOG.info('{} - {} Traffic stats, tx_packets: {}, rx_packets {}.'
                         .format(server['mcast_srv'], server['fip'],
                                 tx_packets,rx_packets))
            if server['mcast_srv'] == "traffic_runner":
                if not (rx_packets >= mcast_packets and rx_packets <= mcast_packets_tolerance):
                    errors.append("No traffic in traffic runner: {}. ".format(rx_packets))
            else:
                if not (tx_packets >= mcast_packets and tx_packets <= mcast_packets_tolerance):
                    errors.append("No traffic in listener: {}. ".format(tx_packets))
