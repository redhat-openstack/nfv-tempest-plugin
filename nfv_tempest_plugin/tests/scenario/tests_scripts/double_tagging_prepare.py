...
logger = logging.getLogger("DoubleTaggingTestConfig")
logger.addHandler(handler)

parser = argparse.ArgumentParser(
    usage='''
    Double tagging test config script
...
parser.add_argument('--install-scapy', help='Install scapy package using pip.',
                    required=False, action='store_true')
...
def main():
    logger.info('Start double tagging test configuration')
    ...
    if args.add_iface:
        ...
        set_virtual_interface(args.port_type, nic, args.vlan, args.addr,
                              args.base_vlan)
        if args.install_scapy:
            logger.info('Installing pip and scapy')
            execute_shell_command(
                'curl https://bootstrap.pypa.io/get-pip.py -o /tmp/get-pip.py')
            execute_shell_command('python /tmp/get-pip.py')
            execute_shell_command('pip install scapy')
    elif args.del_iface:
        remove_virtual_interface(args.port_type, nic, args.vlan,
                                 args.base_vlan)
    logger.info('Double tagging test configuration completed')
