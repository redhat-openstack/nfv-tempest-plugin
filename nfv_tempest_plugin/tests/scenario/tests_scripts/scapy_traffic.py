...
class ScapyTraffic(object):
    ...
    def sniff_parse(self, sniff_output=None):
        if sniff_output is not None:
            pkt_count = len(sniff_output)
            if 'ICMP' in sniff_output[0]:
                pkt_type = sniff_output[0].getlayer(ICMP).type
            else:
                pkt_type = sniff_output[0].type
    ...
    def send_mpls(self):
        ...
        send_mpls_log = mpls_frame.sprintf("Packets send from mac: "
                                           ...
                                           "following raw message: %Raw.load%")
        try:
