...
def get_ifcfg_files():
    ...
    files = fnmatch.filter(os.listdir('/etc/sysconfig/network-scripts/'),
                                      '*ifcfg-*')
...
def osp10_workaround(ifaces,tag):
    """workaround for osp10
       The requisite for this script to work properly is that ifcfg-eth files
       are created by cloud-init. For osp13 and osp16 these files are created,
       but for osp10 only eth0 is created. The reason is the the metadata
       generated by nova only contains 1 network interface. This workaround
