...
def get_ifcfg_files():
    ...
    files = fnmatch.filter(os.listdir('/etc/sysconfig/network-scripts/'),'*ifcfg-*')
    files.remove('ifcfg-lo')
    ifaces = [ file.replace('ifcfg-','') for file in files]
    return ifaces

def osp10_workaround(ifaces,tag):
    """workaround for osp10
       The requisite for this script to work properly is that ifcfg-eth files are
       created by cloud-init. For osp13 and osp16 these files are created, but
       for osp10 only eth0 is created. The reason is the the metadata generated
       by nova only contains 1 network interface. This workaround works in the
       following way:
       1. detect that there are ifcfg-eth files missing
       2. rename this file and configure for the external interface
       3. configure only this interface
       4. for osp13 and osp16 all interfaces are configured
    """
    ...
    for nic in ifaces:
        for nic_name, nic_mac in iter(nic.items()):
            if tag and nic_mac == tag['mac'] and tag['tag'] == args.tag:
                source_file = '/etc/sysconfig/network-scripts/' \
                              'ifcfg-{}'.format(files[0])
                destination_file = '/etc/sysconfig/network-scripts/' \
...
def main():
    ...
    if args.init or not os.path.exists(args.nics_data_path):
        ...
        tag = get_tag()
        logger.info('Check osp10 workaround')
        ifaces = osp10_workaround(ifaces,tag)
