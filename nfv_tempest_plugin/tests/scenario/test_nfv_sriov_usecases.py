...
class TestSriovScenarios(base_test.BaseTest):
    ...
    def test_sriov_double_tagging(self, test='double_tagging', iface_vlan=None,
                                  test_vlan=None):
        """Test SRIOV double tagging functionality

        The test require resource creator to setup initial test resources.
        :param iface_vlan: The vlan that will be set on the virtual interface
        :param test_vlan: The vlan that will be used for the packets send
        ...
        if 'iface_vlan' and 'test_vlan' in \
            ...
            raise ValueError('Test vlans should be set and allowed in the '
        ...
        for num, srv in enumerate(servers):
            ...
            vf_config = 'sudo python /usr/local/bin/nfv_scripts/double_' \
                        'tagging_prepare.py --add-iface --port-type ' \
                        '{port_type} --mac {mac} --vlan {iface_vlan} --addr ' \
                        '{addr}/24 --install-scapy'
        ...
        for srv in servers:
            vf_remove = 'sudo python /usr/local/bin/nfv_scripts/double_' \
                        'tagging_prepare.py --del-iface --port-type vf ' \
                        '--mac {mac} --vlan {iface_vlan}'\
                .format(mac=srv['test_mac_addr'], iface_vlan=iface_vlan)
            pf_remove = 'sudo python /usr/local/bin/nfv_scripts/double_' \
                        'tagging_prepare.py --del-iface --port-type pf ' \
