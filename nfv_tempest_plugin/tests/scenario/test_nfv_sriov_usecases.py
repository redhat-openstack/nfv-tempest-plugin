...
class TestSriovScenarios(base_test.BaseTest):
    ...
    def test_sriov_free_resource(self, test='sriov_reset_resources'):
        ...
        kw_args['filtered_command'] = "sudo ip link show"
        kw_args['file_path'] = \
        ...
        kw_args['nic_req_stat'] = \
            'vf [0-9]{1,3} MAC (?!00:00:00:00:00:00)|trust'\
            ' (?!off)|spoof checking (?!on)'
        ...
        shell_utils. \
            check_required_status_on_hypervisor(**kw_args)
        # Create resources:
