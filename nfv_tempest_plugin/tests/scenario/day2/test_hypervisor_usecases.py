...
class TestHypervisorScenarios(Day2Manager):
    ...
    def test_scale_out_kernelargs_hypervisor_reboot(self, test='scale_out'
        ...
        async_manager = AsyncUtilsManager()
        old_compute, new_compute = self.get_old_and_new_compute()
        self.validate_no_reboot_in_stack_update(hypervisors_ip=old_compute)
        async_manager.multithread_iter_wraper(new_compute,
                                              self.validate_kargs)
        async_manager.multithread_iter_wraper(self.
                                              os_client
                                              .novaclient_overcloud
                                              .hypervisors.list(),
                                              target=self
                                              .reboot_validate_kernel_args)
    ...
    def test_stack_update_kernel_args_hypervisor_reboot(self,
        ...
        async_manager.multithread_iter_wraper(iteratable=self.os_client
                                              .novaclient_overcloud
                                              .hypervisors.list(),
                                              target=self
                                              .reboot_validate_kernel_args)
