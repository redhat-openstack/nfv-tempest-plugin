...
class TestHypervisorScenarios(base_test.BaseTest):
    ...
    def test_no_reboot_in_stack_update(self, stack_name='overcloud'):
        ...
        for hypervisor in hypervisors_ip:
            # TODO: replace with overcloud novaclient Client.uptime (eshulman)
    ...
    def reboot_hypervisor_and_wait(self, hypervisor, novaclient_overcloud):
        ...
        while time.time() < timeout_start + timeout_end:
            time.sleep(10)
            hyper_state = novaclient_overcloud.hypervisors.search(hypervisor_name)[0].state
            if 'down' in hyper_state:
                hyper_rebooted = True
                continue
            if hyper_rebooted and 'up' in hyper_state:
    ...
    def reboot_and_validate_kernel_args(self, hypervisor, novaclient_overcloud):
        expected_args = CONF.nfv_plugin_options
    ...
    def multithread_wrap_validate_kernel_args_after_reboot(self):
        ...
        for hypervisor in novaclient_overcloud.hypervisors.list():
            threads.append(threading.Thread(target=self.reboot_and_validate_kernel_args,
    ...
    def test_update_kernel_argshypervisor_reboot(self, test='update_kernelarg'):
