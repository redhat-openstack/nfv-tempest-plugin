...
class TestHypervisorReboot(base_test.BaseTest):
    def __init__(self, *args, **kwargs):
        super(TestHypervisorReboot, self).__init__(*args, **kwargs)
        self.hypervisor_ip = None

    def setUp(self):
        "Setup a single tenant with an accessible server"
        super(TestHypervisorReboot, self).setUp()
    ...
    def test_hypervisor_reboot(self, test='hypervisor_reboot'):
        ...
        for srv in srv_on_hyper:
            ...
            while True:
                srv_state = self.servers_client.show_server(
                    srv['uuid'])['server']['status']
                if 'ACTIVE' in srv_state:
                    break
