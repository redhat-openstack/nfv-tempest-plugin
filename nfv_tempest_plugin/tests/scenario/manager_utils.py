...
class ManagerMixin(object):
    ...
    def _get_hypervisor_ip_from_undercloud(self, **kwargs):
        ...
        if 'server_id' in kwargs:
            try:
                hypervisor = self.os_admin.servers_client.show_server(kwargs['server_id'])['server']['OS-EXT-SRV-ATTR:hypervisor_hostname']
            except IndexError:
        ...
        hyp=self.os_admin.hypervisor_client.list_hypervisors(detail=True)['hypervisors']
        if hypervisor != "":
            ip_addresses = [ val['host_ip'] for val in hyp if hypervisor.split('.')[0] in val['hypervisor_hostname'] ]
        else:
