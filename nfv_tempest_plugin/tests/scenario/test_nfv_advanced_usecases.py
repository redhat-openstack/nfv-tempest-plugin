...
class TestAdvancedScenarios(base_test.BaseTest):
    ...
    def test_numa_aware_vswitch(self, test='numa_aware_vswitch'):
        ...
        numas_phys = self.locate_numa_physnets(node=numa0_srv[0]['hypervisor'
                                                                 '_ip'],
                                               keys='neutron::agents::ml2'
                                                    '::ovs::bridge_mappings,'
                                                    'nova::compute::neutron_'
                                                    'physnets_numa_nodes_'
                                                    'mapping')
        if not numas_phys['numa_aware_net']:
