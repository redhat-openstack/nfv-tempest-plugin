...
class TestOvsScenarios(base_test.BaseTest):
    ...
    def test_ovs_active_backup_bond_connectivity(self, test='ovs_bond_connectivity',
                                                 requested_bond_type='active-backup'):
        ...
        """
        self.assertNotEmpty(CONF.nfv_plugin_options.ovs_bond_under_test,
        ...
        for server in servers:
            ...
            for bond_name in CONF.nfv_plugin_options.ovs_bond_under_test:
                current_bond = \
                    shell_utils.construct_ovs_bond_tuple_from_hypervsior(
                        hypervisor_ip, bond_name, requested_bond_type)
                if cuurent_bond != None:
                    ovs_bonds.append(current_bond)
            if not ovs_bonds:
                raise ValueError("'No bonds of type '{t}' were found")
                                 .format(t=requested_bond_type)
