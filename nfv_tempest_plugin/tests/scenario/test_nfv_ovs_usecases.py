...
class TestOvsScenarios(base_test.BaseTest):
    ...
    def test_ovs_bond_connectivity(self, test='ovs_bond_connectivity'):
        ...
        # Retrieve all hypervisors in deployment
        hypervisors = self._get_hypervisor_ip_from_undercloud(
            shell='/home/stack/stackrc')
        # Create servers, assuming that scheduler will schedule every instance
        # on different hypervisor
        servers, key_pair = \
            self.create_and_verify_resources(test=test,
                                             num_servers=len(hypervisors))
        ...
        for server in servers:
            ...
            for bond_object in bond_dict:
                current_bond = self.construct_ovs_bond_tuple_from_hypervsior(
