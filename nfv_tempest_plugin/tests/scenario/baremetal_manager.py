...
class BareMetalManager(api_version_utils.BaseMicroversionTest,
    ...
    def read_external_config_file(self):
        ...
        for test in self.external_config['tests-setup']:
            ...
            if 'bonding_config' in test and test['bonding_config'] is not None:
                ...
                if test['name'] not in self.test_setup_dict:
                    self.test_setup_dict[test['name']] = {}
                test = self.test_setup_dict[test['name']]
                if 'config_dict' not in test:
                    test['config_dict'] = {}
                    test['config_dict']['bonding_config'] = \
            ...
            if 'servers' in test and test['servers'] is not None:
                ...
                for item in test['servers']:
                    if 'name' in item.keys() and item['name'] is not None:
                        ...
                        raise ValueError('name required for server config')
                    if 'ports' in item.keys() and item['ports'] is not None:
                        for item2 in item['ports']:
                            if 'network' in item2.keys() and \
                               item2['network'] is not None:
                                servers[item['name']]['ports'] \
                                    .add(item2['network'])
                            else:
                    ...
                    if 'routes' in item.keys() and item['routes'] is not None:
                        for item2 in item['routes']:
                            if 'network' in item2.keys() and \
                                item2['network'] is not None and \
                               'gateway' in item2.keys() and \
                                item2['gateway'] is not None:
                                srv_routes = servers[item['name']]['routes']
                                srv_routes[item2['network']] = \
                                    item2['gateway']
                            else:
                                raise ValueError('missing network and gateway'
                                                 ' in route config')
                self.test_setup_dict[test['name']]['config_dict']['servers'] \
                    = servers
    ...
    def _create_ports_on_networks(self, num_ports=1, servers=None):
        ...
        for nport in range(num_ports):
            networks_list = []
            for net_name, net_param in iter(self.test_network_dict.items()):
                if servers is None or \
                   net_name in servers.items()[nport][1]['ports']:
                    ...
                    if 'port_type' in net_param:
                        ...
                        networks_list.append(net_var) \
                            ...
                            else networks_list.insert(0, net_var)
    ...
    def create_server_with_fip(self, num_servers=1, use_mgmt_only=False,
                               ...
                               servers_config=None, key_pair=None,
        ...
        :param servers_config: servers configuration (netwroks to attach)
        :param key_pair: key_pair data to connect to the servers
        ...
        for num, server in enumerate(servers):
            ...
            LOG.info('The instance - {} is in an {} state'.format(num + 1,
                     srv_state))
            # store ip addresses for each interface of the server
            server['addresses'] = self.os_admin.servers_client \
            ...
            if servers_config is not None:
                # configure routes in the servers
                for net, gw in servers_config.items()[num][1]['routes']\
                    .iteritems():
