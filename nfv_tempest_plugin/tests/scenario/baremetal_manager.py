...
class BareMetalManager(api_version_utils.BaseMicroversionTest,
    ...
    def ping_via_network_namespace(self, ping_to_ip, network_id):
        for net_name, net_param in self.test_network_dict.iteritems():
            if net_name == 'public':
                continue
            network_cli = self.os_admin.networks_client
            net_info = fixed_network.get_network_from_name(net_name,
                                                           network_cli)
            if network_id != net_info['id']:
                break
            cmd = ("sudo ip netns exec qdhcp-" + net_info['id'] +
