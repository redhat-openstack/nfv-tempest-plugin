...
class BareMetalManager(api_version_utils.BaseMicroversionTest,
    ...
    def _create_test_networks(self):
        ...
        for net in self.external_config['test-networks']:
            ...
            if 'transparent_vlan' in net and 'transparent_vlan_parent' in net:
                ...
                self.test_network_dict[net['name']]['transparent_vlan_parent'] = \
                    net['transparent_vlan_parent']
    ...
    def _create_ports_on_networks(self, num_ports=1, **kwargs):
        ...
        :return transparent_ports: transparent poets
        """
        ...
        for net_name, net_param in iter(self.test_network_dict.items()):
            ...
            if 'transparent_vlan' in net_param and \
                ...
                if net_name not in ips_available:
                    ...
                    for index, ip_value in reversed(list(enumate(
                            ips_available[net_name]))):
                        if ip_value not in range(
                                ipaddress.ip_address(net_param['pool_start']),
                                ipaddress.ip_address(net_param['pool_end'])):
                ...
                if net_param['transparent_vlan'] not in \
                    ...
                    transparent_ports[port_index][net_param['transparent_vlan']] = \
                        {'subports': []}

                if not net_param['transparent_vlan_parent']:
                    # add subports
                    transparent = transparent_ports[port_index][net_param['transparent_vlan']]
            ...
            if 'port_type' in net_param:
                ...
                for port_index in range(num_ports):
                    ...
                    # ports_list will be the list of ports used when spawning vms
                    # when using trunk, child  ports must not be in the list
                    # when using transparency, no child port is created, so it
                    ...
                    if trunk_enabled:
                        ...
                        if net_param['trunk_vlan'] not in \
                            ...
                            trunk_ports[port_index][net_param['trunk_vlan']] = \
                                {'subports': []}

                        # add parent port and subports
                        trunk = trunk_ports[port_index][net_param['trunk_vlan']]
                        if trunk_parent:
                    ...
                    if transparent_enabled:
                        transparent = transparent_ports[port_index][net_param['transparent_vlan']]
                        transparent['parent_port'] = port['id']
    ...
    def create_server_with_resources(self, num_servers=1, num_ports=None,
        ...
        if num_servers:
            servers = self.create_server_with_fip(num_servers=num_servers,
                                                  networks=ports_list,
                                                  **kwargs)
            self._configure_vlan_trunk_vms(servers, key_pair, port_list_trunk)
            self._configure_vlan_transparent_vms(servers, key_pair, port_list_transparent)
    ...
    def _configure_vlan_transparent_vms(self, servers, key_pair, port_list_transparent):
            ...
            if len(port_list_transparent[server_index]) > 0:
                ...
                for transparent_name, transparent_value in \
                    ...
                    for subport in transparent_value['subports']:
                        ...
                        mac = ip_command.execute('link', 'show', device)
