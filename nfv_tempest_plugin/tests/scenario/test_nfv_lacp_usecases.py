...
class TestLacpScenarios(base_test.BaseTest):
    ...
    def test_deployment_lacp(self, test='deployment_lacp'):
        ...
        if len(diff_checks_cmd) > 0:
            result.append("Missing checks: {}. Check ovs commands output"
                          "file".format(', '.join(diff_checks_cmd)))
               ...
               check not in diff_checks_cfg:
                if bond_data[check] != bonding_dict[check]:
                    result.append("Check failed: {}. Expected: {}."
                                  " Found: {}".format(check, bonding_dict[key],
                                                      bond_data[key]))
        ...
        * 3 flows: 66% in one interface, 33% in the other one
        ...
        for test in tests:
            ...
            msg = "Traffic not well balanced. Value {} not between the " \
                  "thresholds: {} and {}".format(tk_pks_rel,
                                                 test['threshold_1'],
                                                 test['threshold_2'])

            self.assertTrue(test['threshold_2'] >= tx_pks_rel >=
                            test['threshold_1'], msg)
