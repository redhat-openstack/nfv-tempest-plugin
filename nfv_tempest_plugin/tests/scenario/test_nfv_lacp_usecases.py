...
class TestLacpScenarios(base_test.BaseTest):
    ...
    def test_balance_tcp(self, test='balance_tcp'):
        ...
        * 3 flows: 66% in one interface, 33% in the other one
        ...
        self.assertGreater(len(bonding_dict),0,"Missing configuration, bonding_config not found")
        self.assertGreater(len(data_network), 0, "Missing configuration, data_network not found")
        ...
        server_network = [net for net in srv['addresses'].items() if net[0] == data_network]
        self.assertEqual(len(server_network),1,"VM must have a port connected to {}".format(data_network))
