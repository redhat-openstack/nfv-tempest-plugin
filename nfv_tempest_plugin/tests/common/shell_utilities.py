...
def tcpdump(server_ip, interface, duration, macs=[], protocol=None, port=None,
    ...
    tcpdump_cmd = "date +'%H:%M:%S.0 START_TIME' > {}; sudo nohup timeout {} tcpdump -i {} -nne {} " \
                  ">> {} 2>&1 &".format(file,
                                        duration,
                                        interface,
                                        filters_str,
    ...
    for line in dump.split('\n'):
        ...
        if (len(columns > 0)):
            ...
            if tcpdump_start_timestamp is None:
                ...
                continue
            if start_time is not None and (timestamp - tcpdump_start_timestamp) < start_time:
                flags[0] = False
            if end_time is not None and (timestamp - tcpdump_start_timestamp) > end_time:
                flags[1] = False
