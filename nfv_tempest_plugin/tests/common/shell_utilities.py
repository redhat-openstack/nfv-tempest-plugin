...
def construct_ovs_bond_tuple_from_hypervsior(hypervisor, bond_object):
    ...
    bond_mode_type = 'no_loadbalancing'
    ...
    if bond_mode_type == "no_loadbalancing":
        ...
        raise ValueError('Currenty only supporting bond modes '
                         'that are not set to not load balance '
                         'traffic')
    cmd = 'sudo ovs-vsctl show | grep {} -B100'
    # Fetch OVS general info
    out = run_command_over_ssh(hypervisor, cmd.format(bond_master))
    ovs_bridge_re_filter = r'Bridge ".*"'
    ovs_bridge_re_sub_filter = r'Bridge "(.*)"'
