...
def check_required_status_on_hypervisor(file_path, search_param,
                                        servers_ips, nic_req_stat,
                                        multi_key_values, filtered_command):
    """check_final_status_on_hypervisor

    The method checks for a value [search_param] in received ini_file
    path[file_path]     search for regexp [nic_req_status] and run
    a required command based on regexp [nic_req_status] on any
    command [filtered_command] results
