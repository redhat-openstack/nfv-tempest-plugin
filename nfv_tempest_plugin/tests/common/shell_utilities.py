...
def _get_cpu_details(self, node_ip):
    ...
    output = shell_utils.run_command_over_ssh(node_ip, cmd)
    if output:
        cpu_model = ""
        cpu_flags = []
        params = output.split('\n')
...
def get_cpu_iommu_kernel_arg(self, node_ip):
    ...
    """
    kernel_arg = {}
    cpu_model, cpu_flags = self._get_cpu_details(node_ip)
