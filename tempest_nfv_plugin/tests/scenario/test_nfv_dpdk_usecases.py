...
class TestDpdkScenarios(baremetal_manager.BareMetalManager):
    ...
    def _test_multicast_traffic(self, test_multicast):
        ...
        """
        log_msg = 'Starting multicast test.'
        LOG.info(log_msg)
        print(log_msg)
        ...
        for server in mcast_srv:
            log_msg = 'Booting %s instance.' % server
            LOG.info(log_msg)
            print(log_msg)
        ...
        for key, value in servers.iteritems():
            if ('listener1' in key) or ('listener2' in key):
                log_msg = 'Copying and executing multicast script to %s.' % key
                LOG.info(log_msg)
                print(log_msg)
                ...
                ssh_source.exec_command(
                    'python /tmp/mcast_receive.py -g "224.1.1.1" '
                    '-p 10000 > /tmp/output &')
        """
            ...
            if 'traffic_runner' in key:
                ...
                ssh_source = self.get_remote_client(value['fip'],
                                                    private_key=keypair[
                                                        'private_key'])
                ssh_source.exec_command(
                    'python /tmp/mcast_send.py -g "224.1.1.1" '
                    '-p 10000 -m "mcast_pass"')

        """
        Reading the listeners output files
        """
        get_mcast_results = 'cat /tmp/output'
        for key, value in servers.iteritems():
            if ('listener1' in key) or ('listener2' in key):
                log_msg = 'Reading results from %s instance.' % key
                LOG.info(log_msg)
                print(log_msg)
                ...
                if results != 'mcast_pass':
                    log_msg = '%s unable to receive multicast traffic.' % key
                    LOG.info(log_msg)
                    print(log_msg)
                    return False
                else:
                    log_msg = '%s received multicast traffic.' % key
                    LOG.info(log_msg)
                    print(log_msg)
        log_msg = 'Both listener1 and listener2 received multicast traffic.'
        LOG.info(log_msg)
        print(log_msg)
        return True
    ...
    def test_multicast(self):
        msg = "One of the instances or both of them unable to receive " \
              "multicast traffic. Check log for more details."
