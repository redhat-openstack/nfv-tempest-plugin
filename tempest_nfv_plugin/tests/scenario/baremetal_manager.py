...
class BareMetalManager(manager.ScenarioTest):
    ...
    def read_external_config_file(self):
        ...
        if 'test-flavors' not in self.external_config:
            raise KeyError("Parameter 'test-flavors' "
                           "is missing from the external config file")
        for flavor in self.external_config['test-flavors']:
            missing_param = '{f} flavor missing parameters'.\
                format(f=flavor['name'])
            if 'name' not in flavor:
                raise KeyError(missing_param)
            if 'ram' not in flavor:
                raise KeyError(missing_param)
            if 'disk' not in flavor:
                raise KeyError(missing_param)
            if 'vcpus' not in flavor:
                raise KeyError(missing_param)
